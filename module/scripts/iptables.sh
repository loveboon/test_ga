...
enable_iptables() {
  ...
  $iptables_w -t nat -A ADGUARD_REDIRECT_DNS -d $redir_host_4 -j RETURN || return 1
  ...
  $iptables_w -t nat -A ADGUARD_REDIRECT_DNS -p udp --dport 53 -j DNAT --to-destination $redir_host_4:$redir_port_4 || return 1
  $iptables_w -t nat -A ADGUARD_REDIRECT_DNS -p tcp --dport 53 -j DNAT --to-destination $redir_host_4:$redir_port_4 || return 1
...
enable_ipv6_iptables() {
  ...
  $ip6tables_w -t nat -A ADGUARD_REDIRECT_DNS -d $redir_host_6 -j RETURN || return 1
  ...
  $ip6tables_w -t nat -A ADGUARD_REDIRECT_DNS6 -p udp --dport 53 -j DNAT --to-destination [${redir_host_6}]:$redir_port_6 || return 1
  $ip6tables_w -t nat -A ADGUARD_REDIRECT_DNS6 -p tcp --dport 53 -j DNAT --to-destination [${redir_host_6}]:$redir_port_6 || return 1
...
enable)
  ...
  if [ "$block_ipv6_dns" = true ] || ! check_redir_host_6; then
